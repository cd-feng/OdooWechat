...
const App = {
    ...
    methods: {
        ...
        openWeChartAuthorize(company_data) {
            ...
            const params = `appid=${company_data['wechat_corp_id']}&redirect_uri=${redirectUrl}&response_type=code&scope=snsapi_base&state=${company_data['wechat_corp_id']}&agentid=${company_data['wechat_agent_id']}#wechat_redirect`
            window.location.replace(`https://open.weixin.qq.com/connect/oauth2/authorize?${params}`);
        }
